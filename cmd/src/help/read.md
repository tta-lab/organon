...
--offset is a 1-indexed line offset; omit it to start at line 1. --limit is a
maximum line count; omit it to read all remaining lines. An explicit --limit 0
returns an empty selection with a continuation offset. Both are relative to the
selected content (the whole file, or the exact symbol/section when --symbol-id
is present). Text output follows the Pi 2,000-line / 50-KB
