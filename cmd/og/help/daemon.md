...
the daemon command, log paths, and PATH only. `~/.config/ttal/.env` is still
loaded for non-GitHub services such as Forgejo, without overriding existing
environment variables. Linux writes a systemd user service.
