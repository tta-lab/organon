...
func (c *impriClient) createAction(ctx context.Context, body impriCreateAction) (impriAction, error) {
	if err := validateImpriCreateTitle(body.Title); err != nil {
...
func validateImpriCreateTitle(title string) error {
	if strings.TrimSpace(title) == "" {
		return fmt.Errorf("impri create title must not be blank")
	}
	if strings.ContainsAny(title, "\r\n\u0085\u2028\u2029") {
		return fmt.Errorf("impri create title must be a single line")
	}
	if len([]rune(title)) > maxImpriActionTitleChars {
		return fmt.Errorf("impri create title must be at most %d characters", maxImpriActionTitleChars)
	}
	return nil
}
...
func mergeActionTitle(snapshot PRMergeSnapshot) string {
	prefix := fmt.Sprintf("Approve squash merge: %s/%s#%d", snapshot.Owner, snapshot.Repo, snapshot.PRNumber)
	title := normalizeMergeTitle(snapshot.Title)
	if title != "" {
		prefix += " — " + title
	}
	return truncateMergeTitle(prefix, maxImpriActionTitleChars)
}

func normalizeMergeTitle(title string) string {
...
func truncateMergeTitle(title string, maxChars int) string {
	runes := []rune(title)
...
func markdownMergeTitle(title string) string {
	title = normalizeMergeTitle(title)
	if title == "" {
		title = "(untitled)"
	}
	maxBackticks := 0
